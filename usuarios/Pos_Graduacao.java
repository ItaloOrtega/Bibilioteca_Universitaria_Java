//Pos_Graduacao pertence ao pacote usuarios
package usuarios;
...
//Classe Pos_Graduacao extende Pessoa, ou seja, pega metodos e atributos que ja existem em Pessoa
public class Pos_Graduacao extends Pessoa{

	private String cco;
	//CCO = Código de identificação do Aluno de Pós-Graduação

	public Pos_Graduacao(String name,int age,int quantity,String Email,ArrayList<Livro> books,String id){

		//Essas variaveis dentro do parenteses de super são as variaveis que são originais de Pessoa e que Pos_Graduacao tambem possui
	...
	//Set do CCO da Pessoa
	public void setCCO(String cod){

		this.cco=cod;
	}

	//Get do CCO da Pessoa
	public String getCCO(){

		return this.cco;
	}

	//Metodo para printar as informações das pessoas de Pós-Graduação que estão na Lista
	public void Printar(ArrayList<Pos_Graduacao> G){
	...
	//Esse metodo recebe a Pessoa da Pós-Graduação que tem algum livro reservado
	public ArrayList<Livro> Devolver(Pos_Graduacao P){
	...
	//Recebe a Pessoa da Pós-Graduação e a Lista de Livros cadastrados
	public ArrayList<Livro> Reservar(Pos_Graduacao P,ArrayList<Livro> L ){
		...
		for(int z=0;z<L.size();z++){
			...
			//O ID digitado do usuario sendo encontrado dentro da Lista, faz o livro ser adicionado na Lista de Livros Reservados da classe Pós-Graduação
	...
	//Recebe o arquivo que vai ser escrito, a pessoa da pós-graduação que vai ter as informações escritas
	//O filewriter e o bufferedwriter
	public void adicionarArquivo(File arquivop, Pos_Graduacao pp,FileWriter fw,BufferedWriter bw){

		try{
			//Vai ser escrito o Nome,Idade,E-mail,Quantidade de livros Reservados,os IDs dos livros reservados e por fim o ID da pessoa da pós-graduação
			bw.write(pp.getNome());
			bw.newLine();
	...
	//Metodo para ler o arquivo e adicionar os seus dados na lista de pós-graduação
	//Recebe o arquivo, o a lista de pessoas na pós-graduação e a lista de livros cadastrados
	public ArrayList<Pos_Graduacao> lerArquivo(File arquivop, ArrayList<Pos_Graduacao> P, ArrayList<Livro> L){
		...
		try{
			...
			for(int z=0;z<S.size();z++){
				...
				//Cria uma pessoa na Pós-Graduação generica para receber os dados
				Pos_Graduacao pp = new Pos_Graduacao("",0,0,"",LA,"");

				//A pessoa vai receber os dados na sequencia que está escrito no arquivo
				//A sequencia é nome,idade,e-mail,quantidade de livros reservados, id dos livros reservados e por ultimo o CCO
				pp.setNome(S.get(z));
				z++;
		...
		//Metodo retorna a lista de Pessoas da Pós-Graduação completa 
		return P;
	}
